      ...
      <div v-if="board">
        ...
        <div class="flex justify-between items-center text-gray-500 text-sm mb-6 pb-4 border-b">
          ...
          <span>
            ...
            <template v-if="board.update_date">(수정: {{ formatDate(board.update_date) }})</template>
          </span>
        </div>
        ...
        <div class="flex justify-between items-center mt-8 pt-4 border-t">
          <button @click="goToList" class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">목록</button>
          <div class="space-x-2">
            <button @click="goToEdit" class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">수정</button>
            <button @click="handleDelete" class="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600">삭제</button>
          </div>
        </div>
      </div>
      <div v-else-if="error" class="text-center text-red-500">게시글을 불러오는 중 오류가 발생했습니다.</div>
